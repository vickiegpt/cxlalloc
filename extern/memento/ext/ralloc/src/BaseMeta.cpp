...
template <class T, RegionIndex idx> T &CrossPtr<T, idx>::operator*() {
...
template <class T, RegionIndex idx>
AtomicCrossPtrCnt<T, idx>::AtomicCrossPtrCnt(T *real_ptr,
  ...
  if (real_ptr == nullptr) {
    ...
    res =
        reinterpret_cast<char *>(reinterpret_cast<uint64_t>(res) | cnt_prefix);
  }
  off.store(res);
}
...
template <class T, RegionIndex idx>
inline ptr_cnt<T>
AtomicCrossPtrCnt<T, idx>::load(memory_order order) const noexcept {
...
template <class T, RegionIndex idx>
inline void AtomicCrossPtrCnt<T, idx>::store(ptr_cnt<T> desired,
...
template <class T, RegionIndex idx>
inline bool AtomicCrossPtrCnt<T, idx>::compare_exchange_weak(
    ptr_cnt<T> &expected, ptr_cnt<T> desired, memory_order order) noexcept {
  char *old_off;
  char *new_off;
...
template <class T, RegionIndex idx>
inline bool AtomicCrossPtrCnt<T, idx>::compare_exchange_strong(
    ptr_cnt<T> &expected, ptr_cnt<T> desired, memory_order order) noexcept {
  char *old_off;
  char *new_off;
  uint64_t old_cnt_prefix = expected.cnt << MAX_DESC_OFFSET_BITS;
...
void *BaseMeta::small_sb_alloc(size_t size) {
  ...
  while (true) {
    ...
    if (oldptr) {
      ...
      // below is effectively _rgs->regions[SB_IDX](&tmp_sec_start,PAGESIZE,
      ...
      if (next >
          _rgs->regions[SB_IDX]->base_addr + _rgs->regions[SB_IDX]->FILESIZE) {
  ...
  while (curr_sb < sb_end) {
    ...
    if (anchor.state == SB_EMPTY) {
      ...
      if (curr_desc->heap->sc_idx == 0) {
        ...
        // small sb that's in use
        for (char *free_block = last_possible_free_block;
             free_block < curr_sb + curr_desc->maxcount * curr_desc->block_size;
             free_block += curr_desc->block_size) {
        ...
        if (anchor.count == 0) {
          ...
          // assert((uint64_t)(free_blocks_head - curr_sb)%curr_desc->block_size
          // == 0);
          anchor.avail =
              (uint64_t)(free_blocks_head - curr_sb) / curr_desc->block_size;
          anchor.state = SB_PARTIAL; // it must be SB_PARTIAL already but we
                                     // assign it anyway
...
void GarbageCollection::mark_func_c(char *ptr, size_t tid,
  ...
  if (res == marked_blk.end()) {
    ...
    to_filter_node.push(
        std::pair<char *, size_t>(reinterpret_cast<char *>(addr), tid));
